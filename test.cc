...
int AppMain(int argc, char** argv) {
	...
	{ // GUI Tree
		auto wSurface = new UISurface(ui_root, Rect(100, 400, SCREEN_WIDTH, 400));
		wToggle = new UIToggle(ui_root, Point(0, 0), 200, 100);
		wButton = new UIButton(wSurface, Point(0, 10), 500, 100);
	...
	while ((e = GetEvent()) != EVENT_QUIT) {
		...
		//UIDelegate(e, ui_root, NUMBER_OF(UIWidget*, ui_root));
		UIDraw(scr, ui_root);
		UpdateWindow();
	}
